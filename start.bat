@echo off
echo Starting Rentora Services...

echo Starting API Gateway on port 8000...
start "API Gateway" cmd /k ".\venv\Scripts\activate && uvicorn gateway.main:app --host 127.0.0.1 --port 8000 --reload"

echo Starting Auth Service on port 8001...
start "Auth Service" cmd /k ".\venv\Scripts\activate && uvicorn auth_service.main:app --host 127.0.0.1 --port 8001 --reload"

echo Starting Profile Service on port 8002...
start "Profile Service" cmd /k ".\venv\Scripts\activate && uvicorn profile_service.main:app --host 127.0.0.1 --port 8002 --reload"

echo Starting Property Service on port 8003...
start "Property Service" cmd /k ".\venv\Scripts\activate && uvicorn property_service.main:app --host 127.0.0.1 --port 8003 --reload"

echo Starting Payment Service on port 8004...
start "Payment Service" cmd /k ".\venv\Scripts\activate && uvicorn payment_service.main:app --host 127.0.0.1 --port 8004 --reload"

echo Starting Maintenance Service on port 8005...
start "Maintenance Service" cmd /k ".\venv\Scripts\activate && uvicorn maintenance_service.main:app --host 127.0.0.1 --port 8005 --reload"

echo Starting Onboarding Service on port 8006...
start "Onboarding Service" cmd /k ".\venv\Scripts\activate && uvicorn onboarding_service.main:app --host 127.0.0.1 --port 8006 --reload"

echo Starting Communication Service on port 8007...
start "Communication Service" cmd /k "cd communication_service && npm run start"

echo Starting Billing Service on port 8009...
start "Billing Service" cmd /k ".\venv\Scripts\activate && uvicorn billing_service.main:app --host 127.0.0.1 --port 8009 --reload"

echo Starting Owner Dashboard Service on port 8010...
start "Owner Dashboard Service" cmd /k ".\venv\Scripts\activate && uvicorn owner_dashboard_service.main:app --host 127.0.0.1 --port 8010 --reload"

echo Starting Subscription Service on port 8011...
start "Subscription Service" cmd /k ".\venv\Scripts\activate && uvicorn subscription_service.main:app --host 127.0.0.1 --port 8011 --reload"

echo Starting Support Service on port 8012...
start "Support Service" cmd /k ".\venv\Scripts\activate && uvicorn support_service.main:app --host 127.0.0.1 --port 8012 --reload"

echo Starting Vite Frontend...
start "Frontend" cmd /k "cd frontend && npm run dev"

echo All services started in separate windows!
